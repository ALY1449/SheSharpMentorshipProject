...
const PairingComplete: React.FC<PairingCompleteProps> = ({
  ...
  const [test, setTest] = useState<MahsaResults[]>([]);
  const [menteeName, setMenteeName] = useState<string | null>(null);
  const [mentorName, setMentorName] = useState<string | null>(null);
  const [results, setResults] = useState<JittyResults[]>([]);
      ...
      if (chosen) {
        ...
        try {
          if (participatingAs == "Mentee") {
            ...
            const matchingResponse = await fetch("/backend/api/pair", {
              next: { revalidate: 60 },
            });
            ...
            const matchingResults: MahsaResults[] =
            ...
            const rResponse2 = await fetch("/backend/api/test-pair", {
              next: { revalidate: 60 },
            });
            if (rResponse2.ok) {
              const responseR = await rResponse2.json();
              const filterResponse: JittyResults[] = responseR;
          ...
          if (participatingAs == "Mentor") {
            ...
            const completeStatusArr = [
              {
                url: "/backend/api/put/mentors/completeStatus",
                param: [chosen, matchingResults[0]], // Ensure this structure matches the expected type
              },
            ];

            const rResponse1 = await fetch(
              `/backend/api/get/mentors/pair2?slug=${chosen}`,
              {
                next: { revalidate: 60 },
              }
            );
